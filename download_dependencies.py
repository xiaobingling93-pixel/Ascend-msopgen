...
def update_submodule(args):
    ...
    thirdparty_dependencies, builtin_dependencies = partition(dependencies,
                                                              lambda x: "thirdparty" in x or "third-party" in x)
    if thirdparty_dependencies:
        exec_cmd(["git", "submodule", "update", "--init", "--progress", "--recursive", "--depth=1", "--jobs=4",
                  *thirdparty_dependencies])
    if builtin_dependencies:
        exec_cmd(["git", "submodule", "update", "--init", "--progress", "--remote", "--depth=1", "--jobs=4",
