...
from msopst.common import logger
...
class AscendRTSApi:
    ...
    def __init__(self, simulator_mode: str = None, soc_version: str = None, simulator_lib_path: str = None,
        ...
        self._model_so_list = self._init_model_so_list(simulator_lib_path, soc_version)
    ...
    @staticmethod
    def _init_model_so_list(simulator_lib_path, soc_version):
        if not os.path.isdir(simulator_lib_path):
            raise RuntimeError(f'Simulator lib path is not a folder, path: {simulator_lib_path}')
        release_conf_path = os.path.join(simulator_lib_path, soc_version, "conf", "release_config.json")
        if not os.path.exists(release_conf_path):
            raise RuntimeError(
                "Simulator release config json does not exist, path: %s." % release_conf_path)
        if os.path.getsize(release_conf_path) >= 10000:
            raise RuntimeError(
                "File is larger than expected and at risk of tampering, path: %s." % release_conf_path)
        with open(release_conf_path, 'r') as simulator_file:
            data = json.load(simulator_file)
            if not data or not isinstance(data, dict):
                raise RuntimeError(
                    "The format of the json file is inconsistent with expectations, path: %s." % release_conf_path)
            return data
    ...
    def _load_runtime_so(self):
        def _find_runtime_so():
            ...
            for ld_lib_path in ld_lib_path_list:
                ...
                runtime_so_realpath = os.path.realpath(runtime_so_path_tmp)
                if os.path.exists(runtime_so_realpath):
                    return runtime_so_realpath
            return None
    ...
    def _init_simulator_so_path(self, simulator_mode, soc_version, simulator_lib_path):
        ...
        simulator_so_list = self._model_so_list.get(simulator_mode, [])
        if not simulator_so_list:
            raise RuntimeError("Simulator so corresponding to the simulator mode(%s) is empty." % simulator_mode)
        so_full_path_list = []
    ...
    def _load_simulator_so(self, simulator_mode: str = None, soc_version: str = None, simulator_lib_path: str = None,
        ...
        if simulator_mode not in self._model_so_list.keys():
            raise ValueError(
                "simulator_mode need to be %s, actual is %s" % ("/".join(self._model_so_list.keys()), simulator_mode))
    ...
    def _parse_error_code(self, error_type: int, error_code: int) -> str:
        ...
        if error_type not in rts_info.RT_ERROR_CODE_DICT or error_code >= len(
                rts_info.RT_ERROR_CODE_DICT.get(error_type)):
