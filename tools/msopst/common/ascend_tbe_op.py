...
import numpy as np
from msopst.runtime import AscendRTSApi
from msopst.common import dtype_trans
from msopst.common import shape_utils
...
class AscendOpKernel:
    ...
    def __init__(self, bin_path: str, json_path: str = '', ascendc_cfg=None):
        if not os.path.exists(bin_path):
            raise IOError("bin_path not exist, path: %s" % bin_path)

        if ascendc_cfg is None and not os.path.exists(json_path):
            raise IOError("json_path not exist, path: %s" % json_path)

        self.bin_path = bin_path
        self.json_path = json_path
        if ascendc_cfg is None:
            self._parse_json_file(json_path)
        else:
            self._parse_ascendc_params(ascendc_cfg)
    ...
    @staticmethod
    def _parse_content_by_regex(regex, content):
        parser_info_list = re.findall(regex, content)
        return parser_info_list[0] if parser_info_list else ''

    @staticmethod
    def _parse_op_file(op_path):
        file_util.check_file_valid(op_path)
        with open(op_path, 'r', encoding='utf-8') as file_handler:
            lines = file_handler.readlines()
        func_def_line = ''
        find_func_def_line = False
        for line in lines:
            if not line:
                continue
            line = line.strip()
            ...
            if line.startswith('extern "C"'):
                find_func_def_line = True
                func_def_line += line
                if '(' in func_def_line:
                    break
            elif find_func_def_line:
                func_def_line += line
                if '(' in func_def_line:
                    break
        if not func_def_line:
            return ''
        return AscendOpKernel._parse_content_by_regex(r'void\s+([a-zA-Z0-9_]+)\s*\(', func_def_line).strip()
    ...
    def _parse_ascendc_params(self, cfg):
        self.block_dim = cfg.get('block_dim', 1)
        if self.block_dim < 1:
            raise ValueError('Number of cores executed by the kernel function should be a positive integer.')
        ascendc_op_path = cfg.get('ascendc_op_path')
        self.stub_func_name = self._parse_op_file(ascendc_op_path)
        if not self.stub_func_name:
            raise RuntimeError('Failed to get ascendc kernal_name from op file.')
        self.magic = 'RT_DEV_BINARY_MAGIC_ELF'  # default for ascendc
        self.workspace = []
        # ascendc ut test doesn't support dynamic shape for the time being
        self.has_tiling = False
        self.tiling_data_size = 0
