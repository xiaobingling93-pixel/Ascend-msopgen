#!/bin/bash
# 清除之前遗留的文件
rm -rf *.vcd *.dump *.cce *.log *.bin *.o *pu build
# 不需要TIK打印出内存信息
...
# 指定芯片版本: ascend910, ascend310p, ascend610
SOC_VERSION=$2
if [ ${SOC_VERSION}"x" = "x" ]; then
    echo "ERROR: SOC_VERSION is not specified! please specify ascend910 or ascend610 or ascend310p!"
    exit -1
fi
