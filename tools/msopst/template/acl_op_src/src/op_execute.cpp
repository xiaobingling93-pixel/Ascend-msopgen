...
bool ProcessOutputData(OpRunner &runner)
{
    for (size_t i = 0; i < runner.NumOutputs(); ++i) {
        ...
        bool result;
        if (runner.GetOutputSize(i) == 0) {
            result = WriteFile(filePath, nullptr, 0);
        } else {
            result = WriteFile(filePath, runner.GetOutputBuffer<void>(i), runner.GetOutputSize(i));
        }
        if (!result) {
...
bool OpExecuteInit()
{
    ...
    if (hasInited == false) {
        ...
        std::string result = output + "/result.txt";
        resultFile.open(result.c_str(), std::ios::out);
        ...
        resultFile.close();
        chmod(result.c_str(), S_IRUSR | S_IWUSR | S_IRGRP);
