...
bool OpRunner::InputsInit()
{
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        IF_NOT_SUCCESS_RETURN_FALSE_WITH_ARGS(aclrtMalloc(&devMem, size, ACL_MEM_MALLOC_HUGE_FIRST),
            "Failed to malloc device memory for input[%zu].", i, GetRecentErrMsg());
        devInputs_.emplace_back(devMem);

        void *hostMem = nullptr;
        if (g_isDevice) {
            IF_NOT_SUCCESS_RETURN_FALSE_WITH_ARGS(aclrtMalloc(&hostMem, size, ACL_MEM_MALLOC_HUGE_FIRST),
                "Failed to malloc device memory for input[%zu].", i, GetRecentErrMsg());
        } else {
...
bool OpRunner::OutputsInit()
{
    for (size_t i = 0; i < numOutputs_; ++i) {
        ...
        if (g_isDevice) {
            IF_NOT_SUCCESS_RETURN_FALSE_WITH_ARGS(aclrtMalloc(&hostOutput, size, ACL_MEM_MALLOC_HUGE_FIRST),
                "Failed to malloc device memory for output[%zu].", i, GetRecentErrMsg());
        } else {
